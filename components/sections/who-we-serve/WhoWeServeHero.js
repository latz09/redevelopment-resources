...
const WhoWeServeHero = ({ data }) => {
	...
	return (
		<header className='sticky top-0 z-0 flex flex-col md:grid md:grid-rows-1 md:grid-cols-2 h-[calc(100svh-var(--hero-pt))] gap-[2.56rem] md:gap-0 bg-light'>
			{/* Text panel — content-sized on mobile (pt-5 above it, then the
			    header's gap handles the space before the image), full-height
			    bottom-aligned column starting at md, same as before */}
			<div
		...
		3xl:pl-[max(5rem,calc((100vw-1728px)/2+5rem))]'
			...
			{/* Image panel — fills remaining flex space on mobile,
			    full column height starting at md */}
			<div className='relative flex-1 min-h-0 md:aspect-auto md:h-full'>
				<SanityImage
					image={heroImage}
					alt={headline || 'Who we serve hero image'}
