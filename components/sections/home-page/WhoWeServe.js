...
const SectorCard = ({ data, alt }) => {
	...
	return (
		<div className="bg-dark space-y-2.5 p-1.25 group group hover:bg-light transition duration-500">
			...
			<div>
				<h4 className="text-light group-hover:text-dark transition duration-500">{title}</h4>
				<p className="text-paragraph text-light group-hover:text-dark transition duration-500">{description}</p>
			</div>
			<ButtonLink variant='primary-on-dark' className="group-hover:text-dark group-hover:bg-light transition duration-1000 hover:border-dark">{ctaLabel}</ButtonLink>
		</div>
	);
};